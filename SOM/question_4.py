...
"""
¿Influye el lugar de salida y la distancia recorrida con la tarifa elegida?
"""


class Question_4(Question):
    ...
    def _prepare_question(self):
        ...
        df = self.dc.normalize(dataset[['pickup_longitude', 'pickup_latitude', 'trip_distance', 'ratecode_id']],
                               exclude_columns=['ratecode_id'])
        df = df.sample(frac=1)
        tags = df['ratecode_id'].to_numpy()
        px = df[['pickup_longitude', 'pickup_latitude', 'trip_distance']].to_numpy()
